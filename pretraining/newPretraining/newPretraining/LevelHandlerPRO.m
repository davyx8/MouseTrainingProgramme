...
%       end
     ...
     if ((data(2)==1 && ~wateropen && disconn && trialInit && (choiceLock==2 || choiceLock==0))
        ...
        if ((lickCounter>=lev.lickThresh || lickCounterB>=lev.lickThresh) && toc*1000<=lev.timeWindow && (newValve==valves(1)|| lev.levelNumber==3 ))
    ...
    if ((data(3)==1 && ~wateropen && disconn && trialInit && (choiceLock==3 || choiceLock==0))
        %here we check if lick was preformed AND THAT:
        %~wateropen- water ISNT open
        %disconn -the mouse disconnected the tongue completly
        %so as not to count continous engagements as one lick
        ...
        if ( lickCounterB>=lev.lickThresh && toc*1000<=lev.timeWindow && ( newValve==valves(2)|| lev.levelNumber==3 ))
            %%this is the stage wqe were all waitiing for
            %we check wether:
            %   A.the mouse did enough licks
            %   B. the licks were all done in the specified time window
