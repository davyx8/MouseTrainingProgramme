...
lev.filePath=sprintf('mouseTrainingTest.csv');
lev.lickThresh=3;
...
[samplingTimer dataResults] =...
    LogDataWhenPausing(x,samplingTimer,dataResults);
           end
    ...
    if (data(2)==1 && ~wateropen && disconn && trialInit && toc(valveTimer)<lev.interTrialInterval)
        ...
        if (lickCounter>=lev.lickThresh & toc*1000<=lev.timeWindow & intersect(valveNumber,lev.rewardValve)==valveNumber)
            ...
            [samplingTimer dataResults] =LogDataWhenPausing(x,samplingTimer,dataResults)
            end
            %pause(lev.waterGivePeriod)
           ...
           % pause(0.05);
            ...
            valveopen = pauseWithOlfactionClosing(valveTimer,h2,logger,valveNumber,lev.interTrialInterval,valveOpeningTime,lev,dataResults);
            %dataResults=[]
            insideInterTrial=false;
...
function valveopen = pauseWithOlfactionClosing(valveTimer,h2,logger,valveNumber,interTrialInterval,valveOpeningTime,lev,dataResults)
...
function [samplingTimer dataResults] =LogDataWhenPausing(x,samplingTimer,dataResults)
            data=x.inputSingleScan();
                    if(toc(samplingTimer)>=0.01)
                     dataVec=[data(2); 99];
                     dataResults=[dataResults dataVec];
