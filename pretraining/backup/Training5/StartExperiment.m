...
while (true & closeTraining)
    ...
    if(valveTimer~=0)
        if(toc(valveTimer)<loggingThresh)
            ...
            if(notWrriten)
                ...
                %             if (lickTimer~=0)
                %             if (toc(lickTimer) >lev.timeWindow && wasntLogged)
    ...
    if (data(2)==1 && ~wateropen && disconn && trialInit && toc(valveTimer)<lev.interTrialInterval)
       ...
       % guidata(hObject, handles);
        toc(lickTimer)
        if (lickCounter>=lev.lickThresh & intersect(valveNumber,lev.rewardValve)==valveNumber)
            ...
            valveopen = pauseWithOlfactionClosing(valveTimer,h2,logger,...
                valveNumber,lev.interTrialInterval-toc(valveTimer),valveOpeningTime,lev,dataResults,lickTimer,valveCount,handles);
...
function valveopen = pauseWithOlfactionClosing(valveTimer,h2,logger,valveNumber,...
    interTrialInterval,valveOpeningTime,lev,dataResults,lickTimer,valveCount,handles);
...
while (toc(loopTic)<interTrialInterval)
    ...
    if (toc(lickTimer) >lev.timeWindow && tmp2)
        [waterWasGiven,firsLick,lickCounter,trialInit ] =...
