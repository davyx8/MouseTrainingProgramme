...
if (isempty(lev.randomValves))
    randomValves=generateRandOdor(lev.noRewardValve,lev.rewardValve,lev.numberofTrials);
...
while (true & closeTraining)
    ...
    if(valveTimer~=0)
        if(toc(valveTimer)<loggingThresh)
            ...
            if(notWrriten)
                dlmwrite(lev.lickPath,dataResults(1,:),'-append');
    ...
    if (data(2)==1 && ~wateropen && disconn && trialInit && ...
            toc(valveTimer)<lev.interTrialInterval & ~waterWasGiven)
