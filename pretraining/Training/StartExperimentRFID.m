...
if (lickCounter>=lev.lickThresh)
    if(intersect(valveNumber,lev.rewardValve)==valveNumber)
        ...
        dlmwrite(sprintf('%s_Trials',rfid),[valveCount,valveNumber,1,0,0,0],'-append');
    elseif (intersect(valveNumber,lev.noRewardValve)==valveNumber)
        set(handles.Result, 'String', 'FALSE ALARM');
        dlmwrite(sprintf('%s_Trials',rfid),[valveCount,valveNumber,0,0,1,0],'-append');
    end
else
    if(intersect(valveNumber,lev.rewardValve)==valveNumber)
        dlmwrite(sprintf('%s_Trials',rfid),[valveCount,valveNumber,0,1,0,0],'-append');
        set(handles.Result, 'String', 'MISS');
    elseif (intersect(valveNumber,lev.noRewardValve)==valveNumber)
        set(handles.Result, 'String', 'CORRECT REJECT ');
        dlmwrite(sprintf('%s_Trials',rfid),[valveCount,valveNumber,0,0,0,1],'-append');
    end
end
if(waterWasGiven)
    dlmwrite(sprintf('%s_Water',rfid),[valveCount 1],'-append');
else
    dlmwrite(sprintf('%s_Water',rfid),[valveCount 0],'-append');
end
waterWasGiven=false;
firsLick=true;
lickCounter=0;
